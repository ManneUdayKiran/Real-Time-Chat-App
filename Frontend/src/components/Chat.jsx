...
useEffect(() => {
  ...
  const fetchConversation = async (selected) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/messages/${currentUser}/${selected}`);
      const data = await res.json();
      setMessages(data);
    } catch (err) {
...
const handleDelete = async (id) => {
  ...
  try {
    const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/messages/${id}`, {
      method: 'DELETE',
    });
 ...
 const handleClear = async () => {
  try {
    const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/messages/${currentUser}/${selectedUser}`, {
      method: 'DELETE',
    });
